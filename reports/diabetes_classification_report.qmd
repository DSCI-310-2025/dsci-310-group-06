...
The packages used in this analysis are `tidyverse`, `tidymodels`, `glmnet`, and `ROSE`. Core functions are sourced from the `predictdiabetes` package, with the latest release available [here](https://github.com/DSCI-310-2025/predictdiabetes/releases/latest). The dataset of interest can be acquired from the source by running `dataset_download.py` located in the `~/work/src/` directory. This script will fetch the raw dataset (UCI ID: 891) from the [UC Irvine machine learning repository](https://archive.ics.uci.edu/) and then write the result into a .csv file (`cdc_diabetes_health_indicators.csv`) located in the `~/work/data/raw/` directory.
...
suppressPackageStartupMessages({
  suppressWarnings(library(tidyverse)) # For read_csv()
})
```
