...
format: 
  html:
    ...
    toc-depth: 3
    editor: visual
    bibliography: ~/work/reports/references.bib
execute:
  echo: false
  warning: false
  error: false
...
Since Cramer's V indicates direct associations between each categorical variable and the target, it is limited to linear relationships. Thus, we will need to analyze non-linear correlations as well. Information Gain (IG) is a metric that can be used for feature selection. This measure determines how much uncertainty is reduced when we use a particular feature in the model. A high IG value indicates an informative feature which means that it is useful for prediction. Research has shown positive results when using information gain to select features as a preprocessing step for various machine learning models, including logistic regression (https://doi.org/10.35145/jabt.v5i3.155).
...
df_sorted <- mi %>% arrange(desc(importance))

print(df_sorted)
```

Domain knowledge suggests that an information gain value greater than 0.05 (https://journalofbigdata.springeropen.com/articles/10.1186/s40537-021-00472-4) is generally considered to be valid. This threshold value has been shown to provide high model accuracy and optimal training time across multiple datasets. From the table above, we can see that `GenHlth`, `HighBP`, and `BinnedBMI` have an information gain greater than 0.05.
