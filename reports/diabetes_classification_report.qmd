...
```
There appears to be a sizable portion of people who are categorized as obese (`BinnedBMI` = 4, 5, 6), with this group making up about `r obese_percent`% of the total. Only a slim majority of individuals have a BMI that is not considered obese (`r non_obese_percent`%).
...
As such, we will be using the following features for our logistic regression model: `GenHlth`, `HighBP`, `Age`, `HighChol`, `DiffWalk`, `BinnedBMI`.
