...
The project follows a structured approach to data exploration, preparation, feature selection and classification modeling.
...
From @tbl-diabetes1, we can see that the dataset contains 21 usable features along with the target. However, using every feature in the dataset would not be optimal as having a large number of features in our model could lead to a risk of overfitting, difficulty in interpretation and increased computation time. Thus, we want to select only a subset of relevant predictors from the 21 total risk factors in the dataset.

Bar plots are created below to visually determine correlation between each categorical variable and the target variable `Diabetes_binary`. For `BMI`, a density plot was created due to this feature being numerical.

![Distribution of Diabetes Binary by Various Variables](../output/combined_plots.png){#fig-eda1}

At a glance, it appears that `HighBP`, `HighChol`, `CholCheck`, `Stroke`, `HeartDiseaseorAttack`, `HvyAlcoholConsump`, `DiffWalk`, `Age`, `Education`, `Income`, and `GenHlth` provide the most obvious differences in distribution between their categories and the target (@fig-eda1). There are 14 features with binary categories and 6 features with multiple categories (@fig-eda1). The `BMI` feature is the only numerical one which may cause issues with certain feature selection techniques. As such, we will look to bin this feature into discrete, categorical values later on.
...
# Note: Rudimentary code for testing, NOT meant to be final. Data taken from CDC
bmi_categories <- c(
  "Underweight", 
  "Healthy Weight", 
  "Overweight", 
...
@tbl-diabetes6 shows the resulting counts and percentages of individuals in the dataset that correspond to each binned BMI value.

```{r}
#| label: tbl-diabetes6
#| tbl-cap: Count of each Binned BMI Value in the Dataset
summary_table6 <- binned_df %>%
  group_by(BinnedBMI) %>% 
  summarise(
    Count = n(),
    Percent = round((n() / nrow(binned_df)) * 100, 1)) %>%
  arrange(desc(BinnedBMI))

# Pulls the obese percentage metric
obese_percent <- summary_table6 %>%
  filter(BinnedBMI %in% c(4, 5, 6)) %>%
  summarise(SumPercent = sum(Percent))

# Pulls the non-obese percentage metric
non_obese_percent <- summary_table6 %>%
  filter(BinnedBMI %in% c(1, 2, 3)) %>%
  summarise(SumPercent = sum(Percent))

knitr::kable(summary_table6)
```
...
mi <- information_gain(Diabetes_binary ~ ., data = binned_df)
df_sorted <- mi %>%
  arrange(desc(importance)) %>%
  rename(Variable = attributes, `Information_Gain` = importance)

knitr::kable(df_sorted)
```

Domain knowledge suggests that an information gain value greater than 0.05 is generally considered to be valid for feature selection (@Sholeh2024). This threshold value has been shown to yield high model accuracy and optimal training time across multiple datasets (@Sholeh2024). From @tbl-diabetes8, we can see that `GenHlth`, `HighBP`, and `BinnedBMI` have an information gain greater than 0.05. This makes them suitable candidates for inclusion in the model.

Using both Cramér's V and Information Gain, there is overlap between features selected. This is summarized by @tbl-diabetes9 below:

```{r}
#| label: tbl-diabetes9
#| tbl-cap: Summary of Features Selected using Cramér's V and Information Gain

cramers <- c("GenHlth", "HighBP", "Age", "HighChol", "DiffWalk")
ig <- c("GenHlth", "HighBP", "BinnedBMI", NA, NA)

# Combine them into a data frame
variable_table <- data.frame(
  List1_Variables = cramers,
  List2_Variables = ig
)
...
As such, we will be using the following features for our logistic regression model: `GenHlth`, `HighBP`, `Age`, `HighChol`, `DiffWalk`, `BinnedBMI` (@tbl-diabetes9).
...
The results of our LASSO regression predictions include `lasso_preds`, which provides the predictions for each row, `lasso_probs`, which provides the probability of each classification for each row, and `lasso_metrics`, which displays the following metrics in @tbl-diabetes10:
...
final_model <- readRDS("/home/rstudio/work/output/lasso_tuned_wflow.RDS")

lasso_coefs <- final_model %>%
  workflows::extract_fit_parsnip() %>%
  broom::tidy()

# Select the row with the maximum step for each variable
lasso_coefs_summarized <- lasso_coefs %>%
  arrange(desc(estimate))

library(ggplot2)
ggplot(lasso_coefs_summarized, aes(x = reorder(term, estimate), y = estimate)) +
  geom_bar(stat = "identity") +
  coord_flip() +
  labs(title = "LASSO Model Coefficients", x = "Feature", y = "Coefficient") +
  theme_minimal()
```

From Fig. 3, we can see that multiple levels of `GenHlth` along with `HighBP` and `HighChol` are the main features with positive contributions to the LASSO model. On the flip side, lower age levels are the main features with negative coefficients. For the positive features:

-   `GenHlth`: Individuals who classify themselves as 3 - Good, 4 - Fair, and 5 - Poor seem to predict an increase in the target variable (`Diabetes_binary`)

-   `HighBP`: Individuals with higher than average blood pressure seems to predict an increase in the target variable

-   `HighChol`: Individuals with higher cholesterol levels than average seems to predict an increase in the target variable

For the negative feature: - `Age`: Individuals who fall under 5 (corresponding to ages 25 to 44) seem to predict a decrease in the target variable

From the LASSO coefficients, we can see that vital signs such as high blood pressure and cholesterol along with overall feelings of health are the strongest predictors for diabetes. Additionally, individuals who are younger appear to be inversely correlated with diabetes prevalence. However, as age increases, the corresponding coefficients increase which suggests that being older is a predictor for diabetes.
